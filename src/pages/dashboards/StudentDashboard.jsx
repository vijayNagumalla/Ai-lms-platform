...
import { 
  ...
  User, BarChart3, CheckSquare, BookMarked, Trophy,
  Star, Award, Zap, Rocket, Calendar, ChevronDown, ChevronUp, Code
} from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  // Automatically fetch coding profiles when user logs in
  useEffect(() => {
    if (user?.id) {
      // This will trigger the CodingProgressCard to load data automatically
  ...
  const loadUserData = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Load assessments after user data is available
  ...
  const loadAssessments = async (userId) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const ids = assessmentData.map(a => a.id);
        const duplicateIds = ids.filter((id, index) => ids.indexOf(id) !== index);
        if (duplicateIds.length > 0) {
          console.warn('Found duplicate assessment IDs:', duplicateIds);
        }
        
        // Remove duplicates based on ID and submission status
    ...
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-[95vw] mx-auto space-y-8">
        ...
        <div className="space-y-8">
          ...
          {user?.id ? (
            <CodingProgressCard userId={user.id} />
          ) : (
            ...
            <CardHeader className="bg-muted text-card-foreground rounded-t-lg border-b border-border">
              ...
              <Collapsible defaultOpen={true}>
                <CollapsibleTrigger className="w-full text-left">
                  <div className="flex items-center justify-between p-3 bg-muted rounded-lg cursor-pointer hover:bg-muted/80 transition-colors duration-200">
                    <span className="text-card-foreground font-medium">
                      {assessments.length > 0 ? `${assessments.length} Assessments` : 'No Assessments'}
                    </span>
