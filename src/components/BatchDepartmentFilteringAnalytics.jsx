...
const BatchDepartmentFilteringAnalytics = ({ 
    onBatchDepartmentFilteringAnalyticsViewed,
    showBatchAnalytics = true,
    showDepartmentAnalytics = true,
    showComparativeAnalysis = true,
    showPerformanceTrends = true
}) => {
    const [batchDepartmentAnalytics, setBatchDepartmentAnalytics] = useState(null);
    ...
    useEffect(() => {
        loadBatchDepartmentFilteringAnalytics();
    }, [timeRange, selectedBatch, selectedDepartment]);

    const loadBatchDepartmentFilteringAnalytics = async () => {
        try {
            setIsLoading(true);
            
            const response = await fetch('/api/analytics/batch-department-filtering', {
                method: 'POST',
                ...
                body: JSON.stringify({ 
                    ...
                    selectedDepartment,
                    showBatchAnalytics,
                    showDepartmentAnalytics,
                    showComparativeAnalysis,
                    showPerformanceTrends
                })
            });

            if (!response.ok) throw new Error('Failed to load batch department filtering analytics');
            
            const data = await response.json();
            setBatchDepartmentAnalytics(data.batchDepartmentAnalytics);
            setBatchAnalytics(data.batchAnalytics || []);
            setDepartmentAnalytics(data.departmentAnalytics || []);
            setComparativeAnalysis(data.comparativeAnalysis);
            setPerformanceTrends(data.performanceTrends);
            
            if (onBatchDepartmentFilteringAnalyticsViewed) {
                onBatchDepartmentFilteringAnalyticsViewed(data);
            }
            
        } catch (error) {
            console.error('Error loading batch department filtering analytics:', error);
            toast.error('Failed to load batch department filtering analytics');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const downloadBatchDepartmentAnalytics = async (format = 'pdf') => {
        try {
            const response = await fetch('/api/analytics/download-batch-department-filtering', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    format,
            ...
            a.href = url;
            a.download = `batch_department_analytics_${format}_${Date.now()}.${format}`;
    ...
    if (!batchDepartmentAnalytics) {
        return (
            <div className="text-center py-8">
                <AlertTriangle className="w-12 h-12 text-red-500 mx-auto mb-4" />
                <p className="text-gray-500">No batch department filtering analytics data available</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="text-center">
                <h2 className="text-3xl font-bold text-gray-900">Batch/Department Filtering Analytics</h2>
                <p className="text-gray-600 mt-2">Comprehensive analytics with advanced filtering by batch and department</p>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            >
                                ...
                                {batchDepartmentAnalytics.batches && batchDepartmentAnalytics.batches.map((batch, index) => (
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            >
                                ...
                                {batchDepartmentAnalytics.departments && batchDepartmentAnalytics.departments.map((department, index) => (
            ...
            {/* Batch/Department Analytics Overview */}
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-2xl font-bold text-gray-900">
                                    {batchDepartmentAnalytics.totalBatches}
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-2xl font-bold text-gray-900">
                                    {batchDepartmentAnalytics.totalDepartments}
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className="text-2xl font-bold text-gray-900">
                                    {batchDepartmentAnalytics.totalStudents}
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <p className={`text-2xl font-bold ${getPerformanceColor(batchDepartmentAnalytics.averageScore)}`}>
                                    {batchDepartmentAnalytics.averageScore}%
                                </p>
                            </div>
                        </div>
                    </CardContent>
            ...
            {/* Batch Analytics */}
            {showBatchAnalytics && batchAnalytics.length > 0 && (
                <Card>
                    <CardHeader>
                                ...
                                <div key={index} className="p-4 border rounded-lg">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-medium">{batch.improvement}%</span>
                                        </div>
                                    </div>
            ...
            {showDepartmentAnalytics && departmentAnalytics.length > 0 && (
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center space-x-2">
                                ...
                                <div key={index} className="p-4 border rounded-lg">
                                    <div className="flex items-center justify-between mb-3">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {department.totalStudents} students • {department.totalBatches} batches
                                            </p>
                                        </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex justify-between text-sm">
                                            <span className="text-gray-600">Improvement:</span>
                                            <span className="font-medium">{department.improvement}%</span>
                                        </div>
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="text-center">
                                    <p className="text-2xl font-bold text-purple-600">
                                        {comparativeAnalysis.overallComparison || 0}%
                                    </p>
                                    <p className="text-sm text-gray-600">Overall Comparison</p>
                                </div>
                            </div>
            ...
            <div className="flex justify-center space-x-4">
                <Button 
                    onClick={() => downloadBatchDepartmentAnalytics('pdf')}
                    variant="outline"
                >
                ...
                <Button 
                    onClick={() => downloadBatchDepartmentAnalytics('excel')}
                    variant="outline"
                >
                    <Download className="w-4 h-4 mr-2" />
                    Download Excel
                ...
                <Button 
                    onClick={loadBatchDepartmentFilteringAnalytics}
                    variant="outline"
                >
...
export default BatchDepartmentFilteringAnalytics;
