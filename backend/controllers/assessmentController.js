...
export const createAssessmentTemplate = async (req, res) => {
  try {
    ...
    // Validate assessment_type enum values
    ...
    if (!validAssessmentTypes.includes(assessment_type)) {
      return res.status(400).json({
        success: false,
        message: `Invalid assessment type. Must be one of: ${validAssessmentTypes.join(', ')}`
      });
    }
    ...
    try {
      await pool.execute(
        `INSERT INTO assessment_templates (
          ...
          default_late_submission_minutes, access_control, assignment_settings, sections
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        sqlValues
      );
    } catch (sqlError) {
...
export const getAssessmentTemplates = async (req, res) => {
  try {
    ...
    const [assessments] = await pool.query(
      ...
      LEFT JOIN assessment_assignments aa ON at.id = aa.assessment_id
      ${whereClause}
...
export const updateAssessmentTemplate = async (req, res) => {
  try {
    ...
    if (updateData.status && !['draft', 'active', 'archived'].includes(updateData.status)) {
      return res.status(400).json({
        success: false,
        message: `Invalid status value. Must be one of: draft, active, archived. Received: ${updateData.status}`
      });
    }
    ...
    // Validate assessment_type field if present
    if (updateData.assessment_type && !['quiz', 'test', 'exam', 'assignment', 'coding_challenge', 'survey'].includes(updateData.assessment_type)) {
      return res.status(400).json({
        success: false,
        message: `Invalid assessment_type value. Must be one of: quiz, test, exam, assignment, coding_challenge, survey. Received: ${updateData.assessment_type}`
      });
    }
...
x1
    ...
    await pool.execute(
      `INSERT INTO assessment_questions (
        id, assessment_id, question_id, section_id, question_order, points, time_limit_seconds, is_required
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        assessment_question_id, assessment_id, question_id, section_id, question_order,
        points, time_limit_seconds, is_required
      ]
    );
...
export const sendAssessmentNotifications = async (req, res) => {
  try {
    ...
    console.error('Send assessment notifications error:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
export const sendAssessmentReminder = async (req, res) => {
  try {
    ...
    console.error('Send assessment reminder error:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error'
    });
...
export const getAssessmentQuestions = async (req, res) => {
  try {
    ...
    if (accessResult.length === 0) {
      // Let's check if the assessment exists at all
      const [assessmentCheck] = await pool.query(
        'SELECT * FROM assessment_templates WHERE id = ?',
        [assessment_id]
      );
      
      // Let's check what assignments exist for this assessment
      const [assignmentsCheck] = await pool.query(
        'SELECT * FROM assessment_assignments WHERE assessment_id = ?',
        [assessment_id]
      );
      
      // Let's check the user's college_id
      const [userCheck] = await pool.query(
        'SELECT id, college_id, role FROM users WHERE id = ?',
        [student_id]
      );
    ...
    // Check if there's an in-progress attempt or create one
    const attemptQuery = `
    ...
    if (attemptResult.length === 0) {
      ...
      `, [submissionId, assessment_id, student_id]);
    } else {
      ...
      if (lastAttempt.status === 'submitted' || lastAttempt.status === 'graded') {
        ...
        `, [submissionId, assessment_id, student_id, lastAttempt.attempt_number + 1]);
...
export const saveAssessmentProgress = async (req, res) => {
  try {
    ...
    const [existingResult] = await pool.query(existingQuery, [assessment_id, student_id]);
    ...
    if (existingResult.length > 0) {
      ...
      const insertQuery = `
        INSERT INTO assessment_submissions (
          id, assessment_id, student_id, answers, current_question, 
...
const checkAssessmentExpiration = async (assessmentId, studentId) => {
  try {
    ...
    const [result] = await pool.query(checkQuery, [studentId, assessmentId]);
    
    if (result.length > 0) {
      const { end_date_only, end_time_only, status, started_at, answers } = result[0];
      
      if (end_date_only && end_time_only && status === 'in_progress') {
        const endDateTimeString = `${end_date_only}T${end_time_only}`;
        const endDateTime = new Date(endDateTimeString);
        const now = new Date();
        
        if (now > endDateTime) {
          // Auto-expire this assessment and calculate score if answers exist
          if (answers) {
            ...
            // Calculate time taken from start to end
            const timeTaken = started_at ? Math.round((now - new Date(started_at)) / (1000 * 60)) : 0;
            ...
            const expireQuery = `
              ...
              SET status = 'expired', 
                  ...
                  percentage_score = CASE 
                    WHEN answers IS NOT NULL AND answers != '{}' THEN 
                      (SELECT COUNT(*) * 100.0 / (SELECT COUNT(*) FROM assessment_sections WHERE assessment_id = ?)) 
                    ELSE 0 
                  END
              WHERE assessment_id = ? AND student_id = ? AND status = 'in_progress'
            `;
            
            await pool.query(expireQuery, [timeTaken, assessmentId, assessmentId, studentId]);
          } else {
            ...
            await pool.query(expireQuery, [assessmentId, studentId]);
          }
...
export const startAssessmentAttempt = async (req, res) => {
  try {
    ...
    if (inProgressResult.length > 0) {
      ...
      // Update started_at to current time if it's been more than 30 minutes
      const startedAt = new Date(inProgressResult[0].started_at);
      const timeDiff = (now - startedAt) / (1000 * 60); // minutes
      
      if (timeDiff > 30) {
        await pool.query(
          'UPDATE assessment_submissions SET started_at = NOW() WHERE id = ?',
          [submissionId]
        );
      }
    } else {
...
export const assignAssessmentToStudents = async (req, res) => {
  ...
  try {
    ...
    // Create assessment instances for each student
    const insertPromises = studentRows.map(student => {
      const instanceId = uuidv4();
      return pool.execute(
        `INSERT INTO assessment_instances 
         (id, assessment_template_id, student_id, status, created_at, updated_at) 
         VALUES (?, ?, ?, 'assigned', NOW(), NOW())`,
        [instanceId, assessmentId, student.id]
      );
    });
...
export const sendAssessmentReminders = async (req, res) => {
  const { assessment_id, send_immediately, send_before_start, send_before_end, custom_message } = req.body;
  const { userId } = req.user;

  try {
    // Verify assessment exists and user has permission
    const assessmentQuery = `
      SELECT at.id, at.title, at.start_date, at.end_date, at.created_by
      FROM assessment_templates at
    ...
    const studentsQuery = `
      SELECT u.id, u.email, u.first_name, u.last_name, ai.id as instance_id
      FROM assessment_instances ai
      JOIN users u ON ai.student_id = u.id
      WHERE ai.assessment_template_id = ? AND ai.status = 'assigned'
    `;
    const [studentRows] = await pool.execute(studentsQuery, [assessment_id]);
    ...
    if (send_before_start && assessment.start_date) {
    ...
    if (send_before_end && assessment.end_date) {
